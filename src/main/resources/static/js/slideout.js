					...
					(
							{
								1 : [
										function(require, module, exports) {
											...
											var touch = {
												'start' : msPointerSupported ? 'MSPointerDown'
														: 'touchstart',
												'move' : msPointerSupported ? 'MSPointerMove'
														: 'touchmove',
												'end' : msPointerSupported ? 'MSPointerUp'
														: 'touchend'
											};
											var prefix = (function prefix() {
												var regex = /^(Webkit|Khtml|Moz|ms|O)(?=[A-Z])/;
												var styleDeclaration = doc
												...
												// Nothing found so far? Webkit
												// does not enumerate over the
												// CSS properties of the style
												// object.
												// However (prop in style)
												// returns the correct value, so
												// we'll have to test for
												// the precence of a specific
												// property
											...
											function hasIgnoredElements(el) {
												while (el.parentNode) {
													if (el
															.getAttribute('data-slideout-ignore') !== null) {
														return el;
													}
													el = el.parentNode;
												}
											...
											function Slideout(options) {
												...
												this._touch = options.touch === undefined ? true
														: options.touch && true;
												...
												// Sets classnames
												if (!this.panel.classList
														.contains('slideout-panel')) {
													this.panel.classList
															.add('slideout-panel');
												}
												if (!this.panel.classList
														.contains('slideout-panel-'
												...
												this._duration = parseInt(
														options.duration, 10) || 300;
												this._tolerance = parseInt(
														options.tolerance, 10) || 70;
												this._padding = this._translateTo = parseInt(
														options.padding, 10) || 256;
												this._orientation = this._side === 'right' ? -1
														: 1;
											...
											Slideout.prototype.close = function() {
												...
												setTimeout(
														function() {
															html.classList
																	.remove('slideout-open');
															self.panel.style.transition = self.panel.style['-webkit-transition'] = self.panel.style[prefix
																	+ 'transform'] = self.panel.style.transform = '';
															self.emit('close');
														}, this._duration + 50);
												return this;
											};
											...
											/**
											 * Returns true if the slideout is
											 * currently open, and false if it
											 * is closed.
											 */
											...
											/**
											 * Translates panel and updates
											 * currentOffset with a given X
											 * point
											 */
											Slideout.prototype._translateXTo = function(
													translateX) {
												this._currentOffsetX = translateX;
												this.panel.style[prefix
											...
											Slideout.prototype._setTransition = function() {
												this.panel.style[prefix
														+ 'transition'] = this.panel.style.transition = prefix
														+ 'transform '
														+ this._duration
														+ 'ms ' + this._fx;
												return this;
											...
											Slideout.prototype._initTouchEvents = function() {
												...
												/**
												 * Prevents touchmove event if
												 * slideout is moving
												 */
												this._preventMove = function(
														eve) {
													if (self._moved) {
														eve.preventDefault();
													}
												...
												this._resetTouchFn = function(
													...
													self._preventOpen = (!self._touch || (!self
															.isOpen() && self.menu.clientWidth !== 0));
												};
												...
												this._onTouchEndFn = function() {
													if (self._moved) {
														self
																.emit('translateend');
														(self._opening && Math
																.abs(self._currentOffsetX) > self._tolerance) ? self
																.open()
																: self.close();
													}
													self._moved = false;
												...
												this._onTouchMoveFn = function(
													...
													var dif_x = eve.touches[0].clientX
															- self._startOffsetX;
													...
													if (Math.abs(dif_x) > 20) {
														...
														if (!(self._moved && html.classList
																.contains('slideout-open'))) {
															html.classList
																	.add('slideout-open');
														}

														self.panel.style[prefix
																+ 'transform'] = self.panel.style.transform = 'translateX('
																+ translateX
																+ 'px)';
														self.emit('translate',
																translateX);
											...
											/**
											 * Enable opening the slideout via
											 * touch events.
											 */
											...
											/**
											 * Disable opening the slideout via
											 * touch events.
											 */
											...
											Slideout.prototype.destroy = function() {
												...
												// Return the instance so it can
												// be easily dereferenced
												return this;
											};
										...
										function(require, module, exports) {
											...
											var Emitter = (function() {
												...
												Emitter.prototype.on = function on(
													...
													// Use the current
													// collection of an event or
													// create it.
													this._eventCollection[event] = this._eventCollection[event]
															|| [];

													// Appends the listener into
												 ...
												 * @returns {Object} Returns an
												 *          instance of Emitter.
												 * @example // Remove a given
												 *          listener.
												 *          emitter.off('foo',
												 *          listener);
												 */

												Emitter.prototype.off = function off(
														event, listener) {
												...
												/**
												 * Execute each item in the
												 * listener collection in order
